...
- Draft/public visibility and paid access.
...
- `price_cents` and `monetization_enabled` live on the character record.
- Paid characters include a mandatory 30 user-message trial per buyer and character before checkout is required.
...
- Character purchase creation is idempotent per user and character. If trial messages remain, the purchase endpoint opens chat instead of starting checkout. Razorpay checkout is verified server-side with the payment signature and also supported through the Razorpay webhook path.
...
- Admin monetization operations live at `/app/admin` and API prefix `/v1/admin/monetization`: profile verification, payout processing, RazorpayX refresh, and manual/mock settlement for local testing.
